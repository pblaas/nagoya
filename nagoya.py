...
try:
    ...
    def createSAcert():
        ...
        subprocess.call(["openssl", "rsa", "-in", "sa-" + (args.clustername) + "-k8s-key.pem", "-pubout", "-out", "sa-" + (args.clustername) + "-k8s.pem"], cwd='./tls')
    ...
    def createNodeCert(nodeip, k8srole):
        ...
        if k8srole == "manager":
            ...
            openssltemplate = (etcd_openssl_template.render(
                ipaddress=nodeip,
                loadbalancer=(args.subnetcidr).rsplit('.', 1)[0] + ".3",
                firstmaster=(args.subnetcidr).rsplit('.', 1)[0] + ".10"
            ))
